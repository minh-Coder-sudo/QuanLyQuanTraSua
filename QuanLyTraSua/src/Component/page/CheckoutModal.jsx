...
export default function CheckoutModal({ onClose, onSubmit }) {
    const { addresses, addAddress, selectAddress, selected, fetchAddress } = useAddressStore();
    ...
    // 🔥 LOAD DANH SÁCH ĐỊA CHỈ KHI MODAL MỞ
    useEffect(() => {
        const loadAddresses = async () => {
            try {
                await fetchAddress();
            } catch (err) {
                console.warn('⚠️ Lỗi load danh sách địa chỉ:', err.message);
            }
        };
        loadAddresses();
    }, [fetchAddress]);
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            // 🔥 Fetch lại danh sách từ server để đảm bảo data đồng bộ
            try {
                await useAddressStore.getState().fetchAddress();
            } catch (err) {
                console.warn('⚠️ Lỗi fetch address:', err.message);
            }
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50">
            <div className="bg-white p-6 rounded-2xl w-[420px] shadow-2xl">
                {/* HEADER */}
                <h2 className="text-2xl font-bold mb-4 text-orange-500 text-center">📦 Địa chỉ nhận hàng</h2>

                {/* LIST ADDRESS */}
                <div className="space-y-3 mb-4 max-h-44 overflow-y-auto pr-1">
                    {addresses.length === 0 && <p className="text-center text-gray-400 text-sm">Chưa có địa chỉ nào</p>}
                ...
                <div className="space-y-3">
                    ...
                    <button
                        onClick={handleAdd}
                        disabled={loading}
                        className="w-full bg-orange-500 text-white py-2 rounded-lg hover:bg-orange-600 transition shadow-sm font-medium disabled:opacity-50"
                    >
