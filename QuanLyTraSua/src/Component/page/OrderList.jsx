...
export default function OrderList() {
  ...
  const statusMap = {
    ...
    'PENDING': { label: 'Đang xử lý', class: 'bg-amber-100 text-amber-700' },
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr><td colSpan={6} className="text-center py-20 text-gray-400 animate-pulse">Đang tải lịch sử...</td></tr>
                ...
                <tr key={order._id} className="hover:bg-gray-50/50 transition">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-xs px-2 py-1 bg-gray-100 rounded text-gray-600">
                      {payMap[order.paymentMethod] || 'Tiền mặt'}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${statusMap[order.status]?.class}`}>
