    ...
    <div className="flex h-[calc(100vh-160px)] gap-5 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex gap-2 overflow-x-auto pb-4 shrink-0">
          <TabBtn active={activeCat === 'all'} onClick={() => setActiveCat('all')}>Tất cả</TabBtn>
          {categories.map(c => (
            <TabBtn key={c._id} active={activeCat === c._id} onClick={() => setActiveCat(c._id)}>{c.name}</TabBtn>
          ))}
        </div>

        <div className="flex-1 overflow-y-auto grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 pr-2">
          {loading ? (
            <div className="col-span-full flex items-center justify-center h-40 text-gray-400">Đang tải sản phẩm...</div>
            ...
            <div key={p._id} 
              ...
              <div className="relative aspect-square mb-2 overflow-hidden rounded-lg">
                ...
                {p.status === 'OUT_OF_STOCK' && (
                   <div className="absolute inset-0 bg-black/40 flex items-center justify-center text-white text-xs font-bold">HẾT HÀNG</div>
                )}
              </div>
      ...
      <div className="w-96 bg-white rounded-2xl shadow-xl border border-gray-100 flex flex-col shrink-0 overflow-hidden">
        <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
          ...
          {cart.length > 0 && <button onClick={() => setCart([])} className="text-xs text-red-500 hover:underline">Xóa hết</button>}
        </div>
            ...
            <div key={item.cartId} className="border-b border-gray-50 pb-4 last:border-0">
               <div className="flex justify-between gap-3 mb-2">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-sm text-amber-600">{calculateItemPrice(item).toLocaleString()}đ</p>
                    <div className="flex items-center gap-2 mt-1">
                      <button onClick={() => updateCartItem(item.cartId, { quantity: Math.max(1, item.quantity - 1) })} className="w-5 h-5 rounded-full border border-gray-300 text-gray-500 flex items-center justify-center">-</button>
                      <span className="text-sm font-bold w-4 text-center">{item.quantity}</span>
                      <button onClick={() => updateCartItem(item.cartId, { quantity: item.quantity + 1 })} className="w-5 h-5 rounded-full border border-gray-300 text-gray-500 flex items-center justify-center">+</button>
                    </div>
                  </div>
               </div>
               ...
               <div className="grid grid-cols-2 gap-2 mt-2">
                 <select value={item.sugar} onChange={(e) => updateCartItem(item.cartId, { sugar: parseInt(e.target.value) })} className="text-[10px] border border-gray-200 rounded px-1 py-0.5 bg-gray-50 outline-none">
                    {[100, 70, 50, 30, 0].map(v => <option key={v} value={v}>Đường {v}%</option>)}
                 </select>
                 <select value={item.ice} onChange={(e) => updateCartItem(item.cartId, { ice: parseInt(e.target.value) })} className="text-[10px] border border-gray-200 rounded px-1 py-0.5 bg-gray-50 outline-none">
                    {[100, 70, 50, 30, 0].map(v => <option key={v} value={v}>Đá {v}%</option>)}
                 </select>
               </div>
               
               <button onClick={() => removeFromCart(item.cartId)} className="w-full mt-2 text-[10px] text-red-400 hover:text-red-600 transition">✕ Gỡ bỏ món này</button>
            </div>
          ))}
        </div>
...
function TabBtn({ children, active, onClick }) {
  return (
    <button onClick={onClick} className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition
